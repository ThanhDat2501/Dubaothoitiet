...
function theDifferencecity(theCityJson){
...
async function goiApi() {
    ...
    listCity=["https://api3.vnexpress.net/api/crawler?type=get_data&key=weather_dot_com&province=H%E1%BA%A3i%20Ph%C3%B2ng&app_id=d9b81e&date=2024-07-24","https://api3.vnexpress.net/api/crawler?type=get_data&key=weather_dot_com&province=H%E1%BB%93%20Ch%C3%AD%20Minh&app_id=d9b81e&date=2024-07-24"]
    
    for(let i=0;i<listCity.length;i++){
        const theCityRespon= await fetch(listCity[i])
        const thoiTietJson = await thoiTietResponse.json();
        const statusCode = thoiTietResponse.status;
